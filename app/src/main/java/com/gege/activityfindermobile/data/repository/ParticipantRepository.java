...
public class ParticipantRepository {
     ...
     */
    public void expressInterest(Long activityId, Long userId, ExpressInterestRequest request,
                                ApiCallback<Participant> callback) {
        participantApiService.expressInterest(activityId, userId, request)
                    ...
                    try {
                        if (response.errorBody() != null) {
                            ...
                            if (errorBody.contains("already expressed interest")) {
                                ...
                                int msgStart = errorBody.indexOf("\"message\":\"") + 11;
                            ...
                            errorMsg = "Failed to express interest: " + response.code();
                        }
                    } catch (Exception e) {
                        errorMsg = "Failed to join activity";
    ...
    public void getActivityParticipants(Long activityId, ApiCallback<List<Participant>> callback) {
        participantApiService.getActivityParticipants(activityId)
            ...
            public void onResponse(Call<List<Participant>> call, Response<List<Participant>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    Log.d(TAG, "Fetched " + response.body().size() + " participants");
                    callback.onSuccess(response.body());
                } else {
                    String errorMsg = "Failed to fetch participants: " + response.code();
                    Log.e(TAG, errorMsg);
                    callback.onError(errorMsg);
                }
            }
     ...
     */
    public void getInterestedUsers(Long activityId, Long creatorId,
                                   ApiCallback<List<Participant>> callback) {
        participantApiService.getInterestedUsers(activityId, creatorId)
                .enqueue(new Callback<List<Participant>>() {
            @Override
            public void onResponse(Call<List<Participant>> call, Response<List<Participant>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    Log.d(TAG, "Fetched " + response.body().size() + " interested users");
                    callback.onSuccess(response.body());
                } else {
                    String errorMsg = "Failed to fetch interested users: " + response.code();
                    Log.e(TAG, errorMsg);
                    callback.onError(errorMsg);
                }
            }
    ...
    public void getMyParticipations(Long userId, ApiCallback<List<Participant>> callback) {
        participantApiService.getMyParticipations(userId)
                .enqueue(new Callback<List<Participant>>() {
            @Override
            public void onResponse(Call<List<Participant>> call, Response<List<Participant>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    String errorMsg = "Failed to fetch participations: " + response.code();
                    Log.e(TAG, errorMsg);
                    callback.onError(errorMsg);
                }
            }
     ...
     */
    public void updateParticipantStatus(Long participantId, Long creatorId, String status,
                                       ApiCallback<Participant> callback) {
        ParticipantStatusUpdateRequest request = new ParticipantStatusUpdateRequest(status);
        participantApiService.updateParticipantStatus(participantId, creatorId, request)
                .enqueue(new Callback<Participant>() {
            @Override
            public void onResponse(Call<Participant> call, Response<Participant> response) {
                if (response.isSuccessful() && response.body() != null) {
    ...
    public void leaveActivity(Long activityId, Long userId, ApiCallbackVoid callback) {
        participantApiService.leaveActivity(activityId, userId).enqueue(new Callback<Void>() {
            @Override
            public void onResponse(Call<Void> call, Response<Void> response) {
                if (response.isSuccessful()) {
                    ...
                    String errorMsg = "Failed to leave activity: " + response.code();
                    Log.e(TAG, errorMsg);
                    callback.onError(errorMsg);
                }
            }
