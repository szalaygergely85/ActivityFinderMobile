...
public class ReviewRepository {
     ...
     * Create a review
     */
    public void createReview(Long userId, ReviewCreateRequest request, ApiCallback<Review> callback) {
        reviewApiService.createReview(userId, request).enqueue(new Callback<Review>() {
            @Override
    ...
    public void getReviewsForUser(Long userId, ApiCallback<List<Review>> callback) {
        reviewApiService.getReviewsForUser(userId).enqueue(new Callback<List<Review>>() {
            @Override
            public void onResponse(Call<List<Review>> call, Response<List<Review>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    Log.d(TAG, "Fetched " + response.body().size() + " reviews for user");
                    callback.onSuccess(response.body());
                } else {
    ...
    public void getReviewsForActivity(Long activityId, ApiCallback<List<Review>> callback) {
        reviewApiService.getReviewsForActivity(activityId).enqueue(new Callback<List<Review>>() {
            @Override
            public void onResponse(Call<List<Review>> call, Response<List<Review>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    Log.d(TAG, "Fetched " + response.body().size() + " reviews for activity");
                    callback.onSuccess(response.body());
                } else {
                    String errorMsg = "Failed to fetch activity reviews: " + response.code();
                    Log.e(TAG, errorMsg);
                    callback.onError(errorMsg);
                }
            }
    ...
    public void deleteReview(Long reviewId, Long userId, ApiCallbackVoid callback) {
        reviewApiService.deleteReview(reviewId, userId).enqueue(new Callback<Void>() {
            @Override
