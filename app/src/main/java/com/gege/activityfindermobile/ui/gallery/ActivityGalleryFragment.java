...
public class ActivityGalleryFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        ViewCompat.setOnApplyWindowInsetsListener(view, (v, insets) -> {
            ...
            com.google.android.material.appbar.AppBarLayout appBar = v.findViewById(R.id.app_bar);
            if (appBar != null) {
                appBar.setPadding(
                        0,
                        systemBars.top,
    ...
    private void setupRecyclerView() {
        adapter = new ActivityGalleryAdapter(currentUserId, new ActivityGalleryAdapter.OnPhotoClickListener() {
            @Override
    ...
    private void deletePhoto(ActivityPhoto photo) {
        ...
        activityPhotoRepository.deletePhoto(activityId, photo.getId(), new ApiCallbackVoid() {
            @Override
    ...
    private void handleSelectedPhotos(List<Uri> uris) {
        ...
        if (uris.size() > MAX_PHOTOS) {
            Toast.makeText(
                    requireContext(),
                    "Maximum " + MAX_PHOTOS + " photos allowed. Please select fewer photos.",
                    Toast.LENGTH_LONG).show();
            return;
        }

        // Show confirmation dialog
        String message = uris.size() == 1
